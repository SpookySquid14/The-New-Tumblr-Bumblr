...
// Intensity of the parallax effect when scrolling
...
// --- Noise Functions (Math for randomness) ---
uint hash(uint x) {
    x += (x << 10u);
    x ^= (x >> 6u);
    x += (x << 3u);
...
void main() {
    ...
    for (float starsize = 3.0; starsize > 0.0; starsize -= 0.5) {
        ...
        float core = exp(-lensq); // Bright center
        float glow = 1.0 / (1.0 + lensq); // Soft glow
