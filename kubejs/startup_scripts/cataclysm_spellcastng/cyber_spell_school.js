...
StartupEvents.registry('attribute', event => {
	...
	event.create('cyber_spell_power', 'irons_spells_js:spell').setDefaultValue(1).setMinimumValue(0).setMaximumValue(10)
	event.create('cyber_spell_resistance', 'irons_spells_js:spell').setDefaultValue(1).setMinimumValue(0).setMaximumValue(10)
})
//giving all mobs attributes
EntityJSEvents.attributes(event => {
    event.allTypes.forEach(type => {
        event.modify(type, (a) => {
            a.add('kubejs:cyber_spell_power')
            a.add('kubejs:cyber_spell_resistance')
        })
    })
})
//registering school
StartupEvents.registry('irons_spellbooks:schools', event => {
	...
	event.create('cyber')
		...
		.setFocus('cataclysm_spellcasting:cyber_focus') // The focus needs to be an item tag. You can add this same tag to an item to make it the focus.
		.setPowerAttribute('kubejs:cyber_spell_power') // This sets the power attribute.
		.setResistanceAttribute('kubejs:cyber_spell_resistance') // This sets the resistance attribute.
		...
		.setDamageType('cataclysm_spellcasting:cyber_damage')
})